function gmm = gmm_em_gpu_trans2(dataList, nmix, final_niter, ds_factor, nworkers, gmmFilename,featCol,vadCol,vadThr)
...
cellM = cell(nparts,1);
cellM2 = cell(nparts,1);
cellmu = cell(nparts,1);
cellsigma = cell(nparts,1);
cellw = cell(nparts,1);
...
for ix = 1 : nparts
	...
	cellM2{ix} = dataL2((ix-1)*partDiv+1:end,:);
	end;
end;
...
while ( mix <= nmix )
    ...
    for iter = 1 : niter(log2(mix) + 1)
        ...
        if mix>32 
            ...
            %nparts = 1;
            [n, f, s, ~] = expectation2(dataL,dataL2, gmm.mu,gmm.sigma,gmm.w);
            N = gather(n); F = gather(f); S = gather(s);
        end
