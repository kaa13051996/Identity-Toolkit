function gmm = gmm_em_gpu_trans_spmd(dataList, nmix, final_niter, ds_factor, nworkers, gmmFilename,featCol,vadCol,vadThr)
...
spmd
            ...
            dataL = codistributed(cat(2,dataList{:})',codist);
            dL = getLocalPart(dataL);
            dL2 = dL .* dL;
end;
...
while ( mix <= nmix )
    ...
    for iter = 1 : niter(log2(mix) + 1)
        ...
        if mix>32 
                partDiv = fix(200000/(mix/32));
                nparts = fix(size(dL{1},1) / partDiv);
        else
                nparts = 1;
        end
...
%         for ix = 1 : nfiles,
            ...
            if (ix ~= nparts)
            [n, f, s, l] = expectation2(dL((ix-1)*partDiv+1:ix*partDiv,:),dL2((ix-1)*partDiv+1:ix*partDiv,:), gmm.mu,gmm.sigma,gmm.w);
            else
                [n, f, s, l] = expectation2(dL((ix-1)*partDiv+1:end,:),dL2((ix-1)*partDiv+1:end,:),gmm.mu,gmm.sigma,gmm.w); %, gMu,gSigma,gW);
            end
