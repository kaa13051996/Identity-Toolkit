function gmm = gmm_em_gpu_par(dataList, nmix, final_niter, ds_factor, nworkers, gmmFilename,featCol,vadCol,vadThr)
% GPU version of gmm EM alghorithm, basically used for UBM training
% This function uses multi thread gpu calculations
...
globalT = tic;
% in multi thread version we create gpu array only for small batches.
...
while ( mix <= nmix )
    ...
    for iter = 1 : niter(log2(mix) + 1)
        ...
        %nworkers = 4;
        %if mix>32 
            partDiv = fix(50000/(nworkers*mix/32));
       ...
       % GPUw;
        ...
        if  nparts == 1
            %[N, F, S, l] = expectation2(gpuArray(dataL),gpuArray(dataL2), gmm);   
            [N, F, S, l] = expectation2(dataL,dataL2, gmm);   
            L = sum(l);
...
%         for ix = 1 : nfiles,
           ...
           % GPUw = getCurrentWorker;
            %if ()
            if (ix ~= nparts)
            %[n, f, s, l] = expectation2(gpuArray(dataL(:,(ix-1)*partDiv+1:ix*partDiv)),gpuArray(dataL2(:,(ix-1)*partDiv+1:ix*partDiv)), gmm);
            [n, f, s, l] = expectation2(dataL(:,(ix-1)*partDiv+1:ix*partDiv),dataL2(:,(ix-1)*partDiv+1:ix*partDiv), gmm);
            else
                %[n, f, s, l] = expectation2(gpuArray(dataL(:,(ix-1)*partDiv+1:end)),gpuArray(dataL2(:,(ix-1)*partDiv+1:end)), gmm);
                [n, f, s, l] = expectation2(dataL(:,(ix-1)*partDiv+1:end),dataL2(:,(ix-1)*partDiv+1:end), gmm);
            end
...
function [N, F, S, llk] = expectation2(data, data2, gmm)
% compute the sufficient statistics

%post = lgmmprob(data,gmm.mu, gmm.sigma,gmm.w(:));
muDivSig = gmm.mu./gmm.sigma;
%oneDivSigDat = (1./gmm.sigma)'* data2;
ndim = size(data, 1);
C = sum(gmm.mu.*muDivSig) + sum(log(gmm.sigma));
D = (1./gmm.sigma)' * data2 - 2 * (muDivSig)' * data  + ndim * log(2 * pi);
%D = oneDivSigDat - 2 * (muDivSig)' * data + ndim * log(2 * pi);
post = -0.5 * (bsxfun(@plus, C',  D));
clear C D;
%wait(gpuDevice);
post = bsxfun(@plus, post, log(gmm.w(:)));
